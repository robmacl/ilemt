...
%     Table with sub-cell array of FileName, MetalShape and MetalName.
    ...
    partMap = containers.Map({'full','tip'}, {'Full Navio Handpiece', 'Only Tip'});

    % Create an empty table
    data = table();
    
    %% Find the first number after a word using regular expression
    pattern = '(full|tip)_(deg)([-]?\d+(\.)?(\d+)?)'; %(\d+)';
    ...
    data.Properties.VariableNames = {'FileName', 'NavioParts', 'Orientation'};
    % Display table
    if op_ishigh
        fprintf('\n****************************************************\n')
    ...
    while i <= (size(resultArray, 1))
        ...
        HandpiecePart = data.NavioParts{count};
