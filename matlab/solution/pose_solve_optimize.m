...
if (~isempty(options.concentric_cal_file))
  con_cal = load(options.concentric_cal_file);
else
  con_cal = [];
end
...
parfor (ix = 1:size(couplings, 3))
  ...
  % Starting pose.  Use kim18 concentric solution if available.
  if (~isempty(con_cal))
    pose0 = pose_solve_kim18(Cdes, con_cal, hemisphere(ix));
  else
    % We use the fixture poses to construct the sensor pose
